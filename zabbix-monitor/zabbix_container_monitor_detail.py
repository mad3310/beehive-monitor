...
def check_monitor_detail(serious_dict, general_dict, nothing_dict):
    ...
    for monitor_type in response_value:
        for monitor_item in response_value[monitor_type]:
            ...
            error_record = response_value[monitor_type][
                monitor_item]['error_record']
            return_message = response_value[
                monitor_type][monitor_item]['message']
            if error_record:
                error_record = ", error_record=%s" % str(error_record)
                return_message += str(error_record)
            create_time_str = response_value[
                monitor_type][monitor_item]['ctime']
...
#            print diff
            ...
            if diff > WARNING_TIME_DIFF:
                serious_dict.setdefault(
                    monitor_type_item, "please check async api, the data is out of date!")
