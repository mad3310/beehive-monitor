...
class Container_Opers(Abstract_Container_Opers):
    ...
    def shut_containers_under_oom(self, container_name_list):
        ...
        for container in containers:
            ...
            if not ret:
                logging.error(
                    'container %s under oom value shut down failed' % container)
            result.setdefault(container, ret)
        return result
...
class Container_create_action(Abstract_Async_Thread):
    ...
    def __issue_create_action(self):
        ...
        if self.docker_model.use_ip:
            ...
            if not init_con_ret:
                ...
                failed_flag = {'status': Status.failed,
        ...
        try:
            ...
            if len(r_list) > 0:
                for route in r_list:
                    if route['route_ip'] == real_route:
                        ...
                        child.sendline(r"route del -net 0.0.0.0/%s gw %s dev %s" %
                                       (route['mask_num'], route['route_ip'], route['dev']))
                        child.expect(
                            ["bash", pexpect.EOF, pexpect.TIMEOUT], timeout)
            ...
            if len(r_list) == 0:
                ...
                if has_property(self.docker_model, 'set_network'):
                    child.sendline(
                        "route add -net 172.16.0.0 netmask 255.255.0.0 gw %s dev eth0" % default_gateway)
                    child.expect(
                        ["bash", pexpect.EOF, pexpect.TIMEOUT], timeout)
    ...
    def _get_container_info(self):
        ...
        container_node_info.setdefault(
            'type', self.docker_model.component_type)
    ...
    def __get_route_dicts(self, route_list=None):
        ...
        for line in route_list:
            ...
            if (len(route_ip.split(r'.')) != 4 or len(netmask.split(r'.')) != 4):
                continue
            route_info = {}
...
class Container_start_action(Abstract_Async_Thread):
    ...
    def __issue_start_action(self):
        ...
        self.container_opers.write_container_status_by_containerName(
            self.container_name, start_flag)
        client = Client()
...
class Container_destroy_action(Abstract_Async_Thread):
    ...
    def __issue_destroy_action(self):
        ...
        self.container_opers.write_container_status_by_containerName(
            self.container_name, destroy_flag)
        ...
        exists = self.container_opers.check_container_exists(
            self.container_name)
        ...
        self.container_opers.write_container_status_by_containerName(
            self.container_name, destroy_rst)
