...
class CheckSync():
    ...
    def sync(self):
        ...
        if existed:
            ...
            logging.info(
                "cluster does not exist, may be the first time to sync in a new server cluster")
    ...
    def sync_server_cluster(self):
        ...
        self.config_file_obj.setValue(
            options.server_cluster_property, uuid_value)
    ...
    def sync_data_node(self):
        ...
        if server_ip in server_ip_list:
            ...
            if isinstance(data_node_value, dict):
                self.config_file_obj.setValue(
                    options.data_node_property, data_node_value)
        else:
