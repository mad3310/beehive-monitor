...
#         dict.setdefault("rss_cpu", self.search_pid_family_info(pid))
    ...
    def get_containers_alloc_mem(self):
        ...
        for container_id_iter in containers_id_list:
            ...
            total_mem += self.docker_c.inspect_container(container_id_iter)[
                'Config']['Memory']
        self._logger.info("The memory of all containers is :" +
                          str(total_mem / 1024 / 1024) + "MB")
    ...
    def get_container_disk_load(self, name=""):
        ...
        t_r_w_load = 0.0
        cmd_index = [item for item in range(len(pid_list)) if pid_list[
            item] == 'COMMAND']
...
#
    ...
    def get_container_cpu_load(self):
        ...
        for child in children_pid_list:
            ...
            # print "value : %s" % str(float(pid_sum2 - pid_sum1) /
            # float(sys_sum2 - sys_sum1))
            _percent = float(pid_sum2 - pid_sum1) / float(sys_sum2 - sys_sum1)
            self._logger.info("%s process occupy %s " %
                              (str(child), str(_percent)))
            percent += _percent
    ...
    def get_container_network_load(self):
        ...
        invokeCmd = InvokeCommand()
        # _shell = options.pid_children + " " + str(ppid)
        _shell = """cat /proc/%s/net/netstat | grep 'IpExt: ' | tail -n 1 | awk '{ print $8 "\t" $9 }'""" % str(
            pid)
        ...
        self._logger.info(
            "container process occupy %s network load" % (total_network_load))
        for child in children_pid_list:
            _shell = """cat /proc/%s/net/netstat | grep 'IpExt: ' | tail -n 1 | awk '{ print $8 "\t" $9 }'""" % (
                str(child))
            ret = invokeCmd._runSysCmd(_shell)
            pid_str = ret[0]
            ...
            if pid_list1[0] != "cat:":
                ...
                self._logger.info("%s network load %sBytes" %
                                  (str(child), str(sum2 - sum1)))
                total_network_load += sum2 - sum1
    ...
    def get_container_alloc_mem(self):
        ...
        id = ""
        mem = 0
        # print "containers_id_listcat /proc/39965/net/netstat | grep 'IpExt: '
        # | tail -n 1 | awk '{ print $8 "\t" $9 }' :" , containers_info_list
        if self.name != "":
            for container_info in containers_info_list:
...
#             print container_id_iter
        ...
        #pid = 1
        self._logger.info("Container process occupy %s pages" %
                          (str(container_cur_rss)))
        children_pid_list = self.get_container_children_pid(pid)
...
#         print "size is :", container_cur_rss * page_size
        self._logger.info("Container occupy %s KB" %
                          (str(container_cur_rss * page_size)))
    ...
    def containers_group_resource(self):
        ...
        self._logger.info("Host containers occupy %s KB Memory" %
                          str(containers_cur_rss * page_size))
    ...
    def get_containers_id_pid(self):
        ...
        for container_id_iter in containers_id_list:
            ...
            pid = self.docker_c.inspect_container(
                container_id_iter)['State']['Pid']
            pid_index = "pid_%s" % i
